...
std::string	errorResponse::create200Response(std::map<std::string, std::string> responseHeader, std::string responseBody)
{
	std::string response = "HTTP/1.1 200 OK\r\n" + getCurrentHttpDate() + "Server: " + "webserv\r\n";
	for (std::map<std::string, std::string>::iterator itr = responseHeader.begin(); itr != responseHeader.end(); ++itr)
