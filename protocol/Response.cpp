...
Response::Response(char **envp, Config conf)
    : _envp(envp), _config(conf)
{
}
...
void Response::checkValidity()
{
    ...
    if (dir != NULL)
    {
        ...
        if (this->_request.getLocation().getAutoIndex() == true)
        {
            ...
            this->setResponseHeader("Last-Modified", getCurrentHttpDate());
            this->setResponse(this->_errorResponse.create200Response(
                this->_config.getServerName(), getResponseHeader(), getResponseBody()));
        }
        else
        {
...
void Response::GET_HEAD()
{
    ...
    this->setResponseHeader("Last-Modified", getCurrentHttpDate());
    this->setResponse(this->_errorResponse.create200Response(
        this->_config.getServerName(), this->getResponseHeader(), this->getResponseBody()));
}
...
void Response::TRACE()
{
    ...
    body =
        this->_request.getRequestMethod() + " " + this->_request.getRequestURI() + " HTTP/1.1\r\n";
