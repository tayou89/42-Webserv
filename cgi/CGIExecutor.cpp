...
void CGIExecutor::_createPipeGET(void)
{
    ...
    _nonBlockRead[WRITEFD] = open(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0777);
...
{
    return (_nonBlockRead[READFD]);
}

int CGIExecutor::getWriteFD(void) const
{
    return (_nonBlockWrite[WRITEFD]);
}
