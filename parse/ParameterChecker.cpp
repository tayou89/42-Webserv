...
void ParameterChecker::checkParameter(const std::string &directive, const string_vector &parameters)
{
...
void ParameterChecker::_setCheckFunctions(void)
{
    ...
    _checkFunctions[SERVER_NAME_DIRECTIVE]  = &ParameterChecker::_checkServerNameParameter;
    _checkFunctions[ROOT_DIRECTIVE]         = &ParameterChecker::_checkRootParameter;
    _checkFunctions[ERROR_PAGE_DIRECTIVE]   = &ParameterChecker::_checkErrorPageParameter;
    _checkFunctions[CLIENT_MAX_DIRECTIVE]   = &ParameterChecker::_checkClientMaxParameter;
    _checkFunctions[HEADER_MAX_DIRECTIVE]   = &ParameterChecker::_checkHeaderMaxParameter;
    _checkFunctions[LOCATION_DIRECTIVE]     = &ParameterChecker::_checkLocationParameter;
    _checkFunctions[INDEX_DIRECTIVE]        = &ParameterChecker::_checkIndexParameter;
    _checkFunctions[AUTOINDEX_DIRECTIVE]    = &ParameterChecker::_checkAutoindexParameter;
...
void ParameterChecker::_checkServerParameter(void) const
{
    if (_parameterCount > 0)
        throw(std::invalid_argument("\'server\' directive must not have any parameter"));
}

void ParameterChecker::_checkServerNameParameter(void) const
{
    if (_parameterCount != 1)
        throw(std::invalid_argument("\'server_name\' directive needs one server name"));
}
...
void ParameterChecker::_checkListenParameter(void) const
{
    ...
    if (_parameterCount == 0)
        throw(std::invalid_argument("\'listen\' directive needs \'host:port number\'"));
    if (_parameterCount > 1)
    ...
    if (listenElements.size() != 2)
        throw(std::invalid_argument("\'listen\' parameter must be \'host:port number\'" +
                                    _parameters[0]));
    ...
    if (ConfigUtil::isNumberString(portNumber) == false)
        throw(std::invalid_argument(portNumber + ": Port number must be consist of numbers"));
    if (ConfigUtil::isValidPortNumber(std::atoi(portNumber.c_str())) == false)
        throw(std::invalid_argument(portNumber + ": Port number must be (1 ~ 65535)"));
}
...
void ParameterChecker::_checkRootParameter(void) const
{
    ...
    if (_parameterCount > 1)
        throw(std::invalid_argument("Too many parameters: \'root\' directive need one parameter"));
}
...
void ParameterChecker::_checkClientMaxParameter(void) const
{
    ...
    if (_parameterCount == 0)
        throw(std::invalid_argument("\'client_max_body_size\' directive needs a parameter"));
    if (_parameterCount > 1)
...
void ParameterChecker::_checkHeaderMaxParameter(void) const
{
    ...
    if (_parameterCount == 0)
        throw(std::invalid_argument("\'large_client_header_buffers\' directive needs a parameter"));
    if (_parameterCount > 1)
...
void ParameterChecker::_checkLocationParameter(void) const
{
    ...
    if (firstCharacter != '/' && firstCharacter != '~')
        throw(std::invalid_argument(": \'location\' parameter must start with \'/\' or \'~\'" +
                                    element));
}
...
{
    if (_parameterCount == 0)
        throw(std::invalid_argument("Coudln't find any parameter for \'index\' directive"));
}
...
void ParameterChecker::_checkAutoindexParameter(void) const
{
    ...
    if (_parameterCount == 0)
        throw(std::invalid_argument("\'Autoindex\' directive needs a parameter(\"on\" or \"off\""));
    if (_parameterCount > 1)
        throw(std::invalid_argument(
            "Too many parameters: \'Autoindex\' directive need one parameter"));
    element = _parameters[0];
    if (element != "on" && element != "off")
        throw(std::invalid_argument("Wrong \'Autoindex\' parameter: must be \"on\" or \"off\""));
}

void ParameterChecker::_checkReturnParameter(void) const
{
    if (_parameterCount == 0)
        throw(std::invalid_argument("\'return\' directive needs a parameter(message or URL)"));
    if (_parameterCount > 1)
        throw(
            std::invalid_argument("Too many parameters: \'return\' directive need one parameter"));
}
...
void ParameterChecker::_checkErrorPageParameter(void) const
{
    ...
    if (ConfigUtil::isValidErrorStatusCode(std::atoi(errorNumber.c_str())) == false)
        throw(std::invalid_argument(": " + errorNumber + "error number must be (300 ~ 600)"));
}
...
void ParameterChecker::_checkLimitExceptParameter(void) const
{
    ...
    if (_parameterCount == 0)
        throw(std::invalid_argument("\'limit_except\' directive needs http methods to limit"));
    for (i = 0; i < _parameterCount; i++)
    {
        parameter = _parameters[i];
...
void ParameterChecker::_checkCGIPassParameter(void) const
{
    ...
    if (_parameterCount < 1)
        throw(std::invalid_argument("\'cgi_pass\' needs one parameter(\'on\' or \'off\')"));
    if (_parameterCount > 1)
        throw(std::invalid_argument("Too many parameters: \'cgi_pass\' needs one parameter"));
    element = _parameters[0];
    if (element != "on" && element != "off")
