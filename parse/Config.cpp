...
Config::Config(const Config &object)
{
	*this = object;
}
...
const std::string &Config::getIPAddress(void) const
{
	return (_ipAddress);
}

const int &Config::getPortNumber(void) const
{
	return (_portNumber);
}
...
const Location &Config::getLocation(const std::string &urlPath) const
{
	...
	const_location_iterator bestMatch = endPoint;
	std::string key;
	size_t keyLength, longestLength = 0;
	...
	for (iterator = _locations.begin(); iterator != endPoint; iterator++)
	{
		key = iterator->first;
		keyLength = key.size();
		if (urlPath.compare(0, keyLength, key) == 0 && keyLength > longestLength)
		{
			longestLength = keyLength;
			bestMatch = iterator;
		}
	}
	if (bestMatch == endPoint)
		return (*this);
	else
...
void Config::_setServerName(void)
{
	_serverName = _parameters[0];
}
...
void Config::_setLocationBlock(void)
{
	...
	if (_locationBlocks.find(locationResource) != _locationBlocks.end())
		throw(std::runtime_error("Duplicate location resource: " + locationResource));
...
void Config::_setLocation(void)
{
	...
	for (; iterator != endPoint; iterator++)
		_locations[iterator->first] = Location(iterator->first, iterator->second, *this);
}

void Config::_checkConfigValidation(void) const
{
	if (_ipAddress == "" || _portNumber == 0)
		throw(std::runtime_error("Server configuration doesn't have any listen data"));
	if (_rootDirectory == "")
		throw(std::runtime_error("Server configuration doesn't have a root directory"));
	if (_indexes.size() == 0)
		throw(std::runtime_error("Server configuration doesn't have any index"));
	if (_errorPages.size() == 0)
		throw(std::runtime_error("Server configuration doesn't set any error page"));
}
